"""
base_camera_node — overview/wide-field camera (was index 0 in v2 app).

Publishes:
  /base_camera/image_raw   sensor_msgs/Image

Parameters:
  device_index  int    0       cv2.VideoCapture index (or /dev/video* path)
  width         int    640
  height        int    480
  fps           float  30.0
  frame_id      string "base_camera_link"
"""
import rclpy
...
class BaseCameraNode(Node):
    def __init__(self) -> None:
        super().__init__('base_camera')
        self.declare_parameter('device_index', 0)
        self.declare_parameter('width',        640)
        self.declare_parameter('height',       480)
        self.declare_parameter('fps',          30.0)
        self.declare_parameter('frame_id',     'base_camera_link')
        ...
        if not self._cap.isOpened():
            ...
            self.get_logger().info(f'Base camera opened (index={idx}, {w}x{h} @ {fps}fps)')

        self._pub = self.create_publisher(Image, '/base_camera/image_raw', 10)
